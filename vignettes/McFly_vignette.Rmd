...
We will run all the analysis using occurrence data of avian species from Furnariidae family. This data is a part of the mcfly package, to read it type:
```{r readData, eval=FALSE}
data("Furnariidae")
```
Furnariidae dataset comprises the occurence of 223 Furnariidae species distributed through the entire Neotropical region. The phylogenetic hypothesis used in this analysis was obtained from [BirdTree](https://birdtree.org/) and are presented in phylo_Furnariidae data, also presented in mcfly package.
```{r readPhy, eval=FALSE}
data("phylo_Furnariidae")
```
We also need an environmental descriptor to run mcfly function. In this example we will use a dataframe present in mcfly package that contain different environmental variables that can affect the niche preferences of species (mean anual temperature, temperature sazonality, mean anual precipitation and precipitation sazonality, besides coordinates of communities). 
```{r envdata, eval= FALSE}
data("envir")
```
These three data comprises the basic information needed to perform the individual-based simulation model presented in Mcfly function.

##Data proccessing 

Since large trees requires great computational performance and given that the power of analysis is affected by the representativeness of species pool on phylogenetic hypothesis used, we will split the phylogeny of Furnariidae species, find for subfamilies and select that presented a number of species with a sactisfatory number of occurences in the community data, at the same time that do not make the analysis very time consuming.
```{r phylo_process, eval= FALSE}
sub.tree<- ape::subtrees(phylo_Furnariidae) #subseting phylogeny to family level
...
```
We choose Synallaxinae subfamily, that presented nine species distributed through 649 communities. The occurence of these species was extracted from community data frame. We also filter environmental variables and coordinates of points to match with occurences of Synallaxinae species in community data.
```{r comm_procces, eval= FALSE}
plot(sub.tree[[49]]) #Synallaxinae subfamily
sub.phy<- sub.tree[[49]]
comm.sub.phy<- Furnariidae[, sub.tree[[49]]$tip.label] #selecting only sites that present occurences of species from Synallaxinae subfamily
zero.row<- which(rowSums(comm.sub.phy) == 0)
...
The parameters that need to be set in Mcfly function is defined mainly by the characteristics of niche position of species, that is obtained from a single environmental variable. In this example we decided to use the latitude, assuming that this represent a latent variable that summarizes the variatio in a set of niche dimensions.
The parameters that must be defined previous to run Mcfly function are:

* **sigma**: 
represents the environmental niche breadth and was set as being the standard deviation of environmental variable

* **root.value**:
the trait value at the root of phylogenetic tree, was set as being the mean value of niche dimension of species

* **theta**:
the optimun value of niche position for the species. We defined this value as being the predicted values derived from the relationship among diversity (entropy) and the niche, represented by environmental variable
...
Finally, we choose the alpha parameter that will be used in the OU model to estimate species traits in simulation procedure presented in Mcfly function. The values was choose based on the procedure descripted in the main text.

```{r choos_alpha, eval= FALSE}
OU.alpha<- c(0,0.05,0.25,1)
```

We choose the number of runs, that indicate the number of times that the simulation will be repeated, and the number of computer cores to perform the analysis. Here we set the number of cores as being the total number of cores minus 1, but this must be set accordingly to the configuration of your personal computer
```{r nCores_runs, eval= FALSE}
ncores<- parallel::detectCores() - 1
```

The last parameter to set, **W.r** regulate the dispersion limitation of individuals in the simulated landscape used in individual simulation model (to see more details of simulation model used see [MCSim package](http://rstudio-pubs-static.s3.amazonaws.com/159425_80725873417e42fdb13821c10a198281.html). We used a panmitic community here, by setting W.r value as 0. Other scenarios of dispersal limitation can be used.

Finally, we feed Mcfly function with these parameters
...
This is the statistic that suports the most probable intensity of the evolutionary niche selection that generate the community diversity patterns observed
