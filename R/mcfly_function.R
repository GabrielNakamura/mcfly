...
Mcfly<-function(comm, subset, occurrence= FALSE, env,site.coords, tree,
  ...
  #parallel::clusterExport(CL, c("parallel_PAR", "organize.list"), envir = mcfly)
  parallel::clusterEvalQ(CL,library(MCSim))
  parallel::clusterEvalQ(CL,library(ape))
  parallel::clusterEvalQ(CL,library(geiger))
  res_parallel<- parallel::parLapply(CL, SET, parallel_PAR, OU.alpha.v = OU.alpha, comm = comm, subset = subset, occurrence = occurrence, tree = tree,
  ...
  sigsq.P<- matrix(unlist(lapply(res_parallel, function(x) x[[1]][,1])), nrow= length(OU.alpha), ncol= runs, byrow= FALSE, 
                   dimnames= list(OU.alpha, paste("run", 1:runs)
                   )
  )
  
  K.P<- matrix(unlist(lapply(res_parallel, function(x) x[[1]][,2])), nrow= length(OU.alpha), ncol= runs, byrow= FALSE, 
               dimnames= list(OU.alpha, paste("run", 1:runs)
               )
  )
  
  alpha.P<- matrix(unlist(lapply(res_parallel, function(x) x[[1]][,3])), nrow= length(OU.alpha), ncol= runs, byrow= FALSE, 
                   dimnames= list(OU.alpha, paste("run", 1:runs)
                   )
  )
  
  z0.P<- matrix(unlist(lapply(res_parallel, function(x) x[[1]][,4])), nrow= length(OU.alpha), ncol= runs, byrow= FALSE, 
                dimnames= list(OU.alpha, paste("run", 1:runs)
                )
  )
  
  
  mean.ent.1<-matrix(NA, nrow=nrow(comm), ncol= length(OU.alpha), dimnames=
                       list(rownames(comm),c(paste("alpha",OU.alpha,sep="="))))
  mean.ent.2<-matrix(NA,nrow=nrow(comm),ncol=length(OU.alpha),dimnames=
                       list(rownames(comm),c(paste("alpha",OU.alpha,sep="="))))
  mean.ent.12<-matrix(NA,nrow=nrow(comm),ncol=length(OU.alpha),dimnames=
                        list(rownames(comm),c(paste("alpha",OU.alpha,sep="="))))
  
  for(i in 1:nrow(comm)){
    mean.ent.1[i,]<- apply(matrix(unlist(lapply(res_parallel, function(x) x[[2]][,i]))
                                  ,nrow= runs, ncol= length(OU.alpha), dimnames= list(paste("run",1:runs,sep=),paste("alpha",OU.alpha,sep="=")),byrow=T),2, mean)
    mean.ent.2[i,]<- apply(matrix(unlist(lapply(res_parallel, function(x) x[[3]][,i]))
                                  ,nrow= runs, ncol= length(OU.alpha), dimnames= list(paste("run",1:runs,sep=),paste("alpha",OU.alpha,sep="=")),byrow=T),2, mean)
    mean.ent.12[i,]<- apply(matrix(unlist(lapply(res_parallel, function(x) x[[4]][,i]))
                                   ,nrow= runs, ncol= length(OU.alpha), dimnames= list(paste("run",1:runs,sep=),paste("alpha",OU.alpha,sep="=")),byrow=T),2, mean)
    
  }
  
  #####AIC selection#####
  AIC.mod<-matrix(NA,length(OU.alpha),ncol=3,dimnames=list(c(paste("alpha",OU.alpha,sep="=")),paste(c("Ent.1","Ent.2","Ent.12"))))
  for (k in 1:length(OU.alpha)){
    AIC.mod[k,1]<- stats::AIC(stats::lm(obs.ent.1~mean.ent.1[,k]))
    AIC.mod[k,2]<- stats::AIC(stats::lm(obs.ent.2~mean.ent.2[,k]))
    AIC.mod[k,3]<- stats::AIC(stats::lm(obs.ent.12~mean.ent.12[,k]))
  }
  R2<-matrix(NA,length(OU.alpha),ncol=3,dimnames=list(c(paste("alpha",OU.alpha,sep="=")),paste(c("Ent.1","Ent.2","Ent.12"))))
  for (k in 1:length(OU.alpha)){
    R2[k,1]<-summary(stats::lm(obs.ent.1~mean.ent.1[,k]))$r.squared
    R2[k,2]<-summary(stats::lm(obs.ent.2~mean.ent.2[,k]))$r.squared
    R2[k,3]<-summary(stats::lm(obs.ent.12~mean.ent.12[,k]))$r.squared
  }
  W<-matrix(NA,length(OU.alpha),ncol=3,dimnames=list(c(paste("alpha",OU.alpha,sep="=")),paste(c("Ent.1","Ent.2","Ent.12"))))
  for (l in 1:ncol(AIC.mod)){
    W[,l]<-MuMIn::Weights(AIC.mod[,l])
  }
  
  K.mean<-matrix(NA,nrow=length(OU.alpha),ncol=runs,dimnames=
                   list(c(paste("alpha",OU.alpha,sep="=")),1:runs))
  alpha.mean<-matrix(NA,nrow=length(OU.alpha),ncol=runs,dimnames=
                       list(c(paste("alpha",OU.alpha,sep="=")),1:runs))
  sigsq.mean<-matrix(NA,nrow=length(OU.alpha),ncol=runs,dimnames=
                       list(c(paste("alpha",OU.alpha,sep="=")),1:runs))
  z0.mean<-matrix(NA,nrow=length(OU.alpha),ncol=runs,dimnames=
                    list(c(paste("alpha",OU.alpha,sep="=")),1:runs))
  for(j in 1:runs){
    K.mean[,j]<-apply(K.P[,sample(1:ncol(K.P),size=runs,replace=TRUE),
                          drop = FALSE],1,mean)
    alpha.mean[,j]<-apply(alpha.P[,sample(1:ncol(alpha.P),size=runs,
                                          replace=TRUE), drop = FALSE],1,mean)
    sigsq.mean[,j]<-apply(sigsq.P[,sample(1:ncol(sigsq.P),size=runs,
                                          replace=TRUE), drop = FALSE],1,mean)
    z0.mean[,j]<-apply(z0.P[,sample(1:ncol(z0.P),size=runs,replace=TRUE),
                            drop=FALSE],1,mean)
  }
  K.mean.boot<-matrix(NA,nrow=3,ncol=length(OU.alpha),dimnames=
                        list(c("lower","mean","upper"),c(paste("alpha",OU.alpha,sep="="))))
  ...
  alpha.mean.boot<-matrix(NA,nrow=3,ncol=length(OU.alpha),dimnames=
                            list(c("lower","mean","upper"),c(paste("alpha",OU.alpha,sep="="))))
  ...
  sigsq.mean.boot<-matrix(NA,nrow=3,ncol=length(OU.alpha),dimnames=
                            list(c("lower","mean","upper"),c(paste("alpha",OU.alpha,sep="="))))
  ...
  z0.mean.boot<-matrix(NA,nrow=3,ncol=length(OU.alpha),dimnames=
                         list(c("lower","mean","upper"),c(paste("alpha",OU.alpha,sep="="))))
  ...
  Res<-list(Entropy=obs.ent,Predicted.entropy.1=mean.ent.1,Predicted.entropy.2=mean.ent.2,Predicted.entropy.12=mean.ent.12,K.niche.position=K.mean.boot,Alpha.niche.position=alpha.mean.boot,
            Sigsq.niche.position=sigsq.mean.boot,Z0.niche.position=z0.mean.boot,AIC=AIC.mod,W=W,R2=R2)
  return(Res)
}
